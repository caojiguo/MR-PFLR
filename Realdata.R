...
ggsave(p, filename = "BetaEst.eps", width = 8, height = 6)

ggsave(beta1_MR, file = "beta1_MR.eps", width = 8, height = 6)
ggsave(beta1_MR, file = "beta1_MR.pdf", width = 8, height = 6)
ggsave(beta1_AD, file = "beta1_AD.eps", width = 8, height = 6)
ggsave(beta1_AD, file = "beta1_AD.pdf", width = 8, height = 6)
ggsave(beta1_RF, file = "beta1_RF.eps", width = 8, height = 6)
ggsave(beta1_RF, file = "beta1_RF.pdf", width = 8, height = 6)
ggsave(beta2_MR, file = "beta2_MR.eps", width = 8, height = 6)
ggsave(beta2_MR, file = "beta2_MR.pdf", width = 8, height = 6)
ggsave(beta2_AD, file = "beta2_AD.eps", width = 8, height = 6)
ggsave(beta2_AD, file = "beta2_AD.pdf", width = 8, height = 6)
ggsave(beta2_RF, file = "beta2_RF.eps", width = 8, height = 6)
ggsave(beta2_RF, file = "beta2_RF.pdf", width = 8, height = 6)
...
#  d <- length(W)
#  m <- ncol(Y)
#  n_train <- floor(nrow(Y) * 0.9)
#  n_test <- nrow(Y) - n_train
#  Y_train_ind <- sample(1 : nrow(Y), n_train)
#  Y_train <- Y[Y_train_ind, ]
#  Y_test <- Y[-Y_train_ind, ]
#  W_train <- list()
#  W_test <- list()
#  W_train$left_thalamus <- W$left_thalamus[Y_train_ind, ]
#  W_train$right_lateral_ventricle <- W$right_lateral_ventricle[Y_train_ind, ]
#  W_test$left_thalamus <- W$left_thalamus[-Y_train_ind, ]
#  W_test$right_lateral_ventricle <- W$right_lateral_ventricle[-Y_train_ind, ]
#  Z_train <- Z[Y_train_ind, ]
#  Z_test <- Z[-Y_train_ind, ]
#  result.fts <- FTSfun(Y_train, W_train, Z_train, W.time, c(5, 5))
#  result.pls <- PLSfun(Y_train, W_train, Z_train, W.time, c(5, 5))
#  beta_pls <- result.pls$beta
#  beta_fts <- result.fts$beta
#  alpha_pls <- result.pls$alpha
#  alpha_fts <- result.fts$alpha
#  fx_pls <- array(NA, c(n_test, m, d))
#  fx_fts <- array(NA, c(n_test, m, d))
#  Y_ipls <- matrix(NA, nrow = n_test, ncol = ncol(Y))
#  for (j in 1 : d){
#   fx_pls[ , , j] <- W_test[[j]] %*% t(beta_pls[[j]]) * (max(W.time[[j]]) - min(W.time[[j]])) / m
#   fx_fts[ , , j] <- W_test[[j]] %*% t(beta_fts[[j]]) * (max(W.time[[j]]) - min(W.time[[j]])) / m
#   for (i in 1 : ncol(Y)){
#    result_ipls <- PLSfun(Y_train[, i], W_train, Z_train, W.time, c(5, 5))
#    beta_ipls <- result_ipls$beta
#    alpha_ipls <- result_ipls$alpha
#    fx_ipls <- W_test[[j]] %*% t(beta_ipls[[j]]) * (max(W.time[[j]]) - min(W.time[[j]])) / m
#    Y_ipls[, i] <- apply(fx_ipls, c(1, 2), sum) + Z_test %*% alpha_ipls
#   }
#  }
#  Y_pls <- apply(fx_pls, c(1, 2), sum) + Z_test %*% alpha_pls
#  Y_fts <- apply(fx_fts, c(1, 2), sum) + Z_test %*% alpha_fts
#  PE_pls <- apply(abs(Y_pls - Y_test), 2, mean)
#  PE_fts <- apply(abs(Y_fts - Y_test), 2, mean)
#  PE_ipls <- apply(abs(Y_ipls - Y_test), 2, mean)
# }
