...
fts_data1 <- read.csv("AR_result_fts_n200.csv", header = T)
Orts_data1 <- read.csv("AR_result_Orts_n200.csv", header = T)
pls_data1 <- read.csv("AR_result_pls_n200.csv", header = T)
Orls_data1 <- read.csv("AR_result_Orls_n200.csv", header = T)

rho <- rep(pls_data1$rho, 4)
methods <- rep(c("FPLS", "FPLS-Oracle", "FPWLS", "FPWLS-Oracle"), each =length(pls_data1$rho))
MSEf_value1 <- c(pls_data1$MSEf_mean, Orls_data1$MSEf_mean, 
                fts_data1$MSEf_mean, Orts_data1$MSEf_mean)
MSEs_value1 <- c(pls_data1$MSEs_mean, Orls_data1$MSEs_mean, 
                fts_data1$MSEs_mean, Orts_data1$MSEs_mean)
...
fts_data2 <- read.csv("EX_result_fts_n200.csv", header = T)
Orts_data2 <- read.csv("EX_result_Orts_n200.csv", header = T)
pls_data2 <- read.csv("EX_result_pls_n200.csv", header = T)
Orls_data2 <- read.csv("EX_result_Orls_n200.csv", header = T)

MSEf_value2 <- c(pls_data2$MSEf_mean, Orls_data2$MSEf_mean, 
                 fts_data2$MSEf_mean, Orts_data2$MSEf_mean)
MSEs_value2 <- c(pls_data2$MSEs_mean, Orls_data2$MSEs_mean, 
                 fts_data2$MSEs_mean, Orts_data2$MSEs_mean)
...
## plot of PE ##
EX_PE_plot <- ggplot(EX_data, aes(x = factor(rho), y = PE_value, 
                          group = methods, linetype = methods, 
                          shape = methods, color = methods))+
  geom_line(size = 0.6)+ 
  geom_point(size = 1.7)+
...
ggsave(p, filename = "plotn200.pdf", width = 14, height = 9)
